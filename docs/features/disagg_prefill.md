...
- **OffloadingConnector**: enable offloading of KV data to CPU memory or SSD. 
  
  For CPU offloading, customize the CPU block size (in tokens) and number of blocks to allocate (per worker):
  ...
  For SSD offloading using GPU Direct Storage (GDS), see [SSD Offloading](ssd_offloading.md):

  ```bash
  --kv-transfer-config '{"kv_connector":"OffloadingConnector","kv_role":"kv_both","kv_connector_extra_config":{"spec_name": "SSDOffloadingSpec", "num_ssd_blocks": 10000, "ssd_cache_dir": "/mnt/nvme/vllm_cache"}}'
  ```
